...
%}
  ...
  e_var = s_var;
    ...
    if ~exist('sun_time', 'var')
        sun_time = generate_sun_array(meta.Year,48,meta);
    end
...
%             end
        ...
        check_start = check_start(check_start(:,1) ~= 0, :, :);
        check_stop  = check_stop (check_stop (:,1) ~= 0, :, :);
  ...
  figure(100)
  hold on
  for u = 1:size(sun_vecs,1)
  quiver3(0,0,0,sun_vecs(u,1,1),sun_vecs(u,2,1),sun_vecs(u,3,1))
  end
        i = 1;
  ...
  hold on
  for u = 1:size(sun_vecs,1)
  quiver3(0,0,0,sun_vecs(u,1,2),sun_vecs(u,2,2),sun_vecs(u,3,2))
  end
  close 100 101
