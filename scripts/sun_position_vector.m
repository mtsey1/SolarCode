function sun = sun_position_vector(time, location, as_array_given)
...
%   This can also be either a vector of such structures, or a structure in
%   which all elements are arrays of the same size (or size 1).
%
...
%   This can also be either a vector of such structures, or a structure in
%   which all elements are (row) vectors of the same size (or size 1).
%   If both  location  and  time  are vectors, the results are unpredictable.
% 
% Output parameters
...
%   24/07/2015  Vectorize to allow times to be a structure array

as_array = false;

if length(time) > 1
  as_array = true;	% flag to return as array of structures
  	% convert to structure of row vector arrays, not array of structures
  t1.UTC   = [time.UTC];
  t1.year  = [time.year];
  t1.month = [time.month];
  t1.day   = [time.day];
...
if length(location) > 1
  as_array = true;
  ll.longitude = [ll.longitured]';	% convert to struct of column vectors
  ll.latitude  = [ll.latitude]';
  ll.altitude = [ll.altitude]';
  location = ll;
end

if nargin >= 3
  as_array = as_array_given;
end

sizes = [length(time.UTC), length(time.year), length(time.month), length(time.day), length(time.hour), length(time.min), length(time.sec)];
non_scalar = sizes (sizes > 1);
if ~isempty(non_scalar) && any (non_scalar ~= non_scalar(1))
  error('sun_position: time components must all have equal size');
end
sizes = [length(location.longitude), length(location.latitude), length(location.altitude)];
if ~isempty(non_scalar) && any (non_scalar ~= non_scalar(1))
  error('sun_position: location components must all have equal size');
end
...
if as_array
  s1 = struct('zenith', mat2cell(sun.zenith, 1, ones(length(sun.zenith),1)));
  s2 = struct('azimuth',mat2cell(sun.azimuth,1, ones(length(sun.zenith),1)));
  [s1(1:length(sun.azimuth)).azimuth] = deal(s2.azimuth);
  sun = s1;
end
...
% This function computes the julian day and julian century from the local
...
% On 4 October 1582, the gregorian calendar was adopted
preG = ([time.year] < 1582) | ([time.year] == 1582 & ([time.month] < 10 | [time.month] == 10 & [time.day] < 15));
A = floor(Y/100);
B(~preG) = 2 - A + floor(A/4);
B(preG) = 0;
%if(time.year == 1582)
%    if(time.month == 10)
...
JME = julian.ephemeris_millenium(:)';	% force to row vector
...
%L5 =     A5 .* cos(B5 + (C5 * JME));
L0 = sum(bsxfun(@times, A0, cos(bsxfun(@plus, B0, bsxfun(@times, C0, JME)))),1);
L1 = sum(bsxfun(@times, A1, cos(bsxfun(@plus, B1, bsxfun(@times, C1, JME)))),1);
L2 = sum(bsxfun(@times, A2, cos(bsxfun(@plus, B2, bsxfun(@times, C2, JME)))),1);
L3 = sum(bsxfun(@times, A3, cos(bsxfun(@plus, B3, bsxfun(@times, C3, JME)))),1);
L4 = sum(bsxfun(@times, A4, cos(bsxfun(@plus, B4, bsxfun(@times, C4, JME)))),1);
L5 =     bsxfun(@times, A5, cos(bsxfun(@plus, B5, bsxfun(@times, C5, JME))));
...
L2 = sum(bsxfun(@times, A2, cos(bsxfun(@plus, B2,(bsxfun(@times, C2,JME))))),1);
L3 = sum(bsxfun(@times, A3, cos(bsxfun(@plus, B3,(bsxfun(@times, C3,JME))))),1);
L4 =     bsxfun(@times, A4, cos(bsxfun(@plus, B4,(bsxfun(@times, C4,JME)))));
...
X4 = ((p(1) * JCE + p(2)) .* JCE + p(3)) .* JCE + p(4); 
...
delta_longitude = (bsxfun(@plus, nutation_terms(:,1), (nutation_terms(:,2) * JCE))) .* sin(tabulated_argument);
delta_obliquity = (bsxfun(@plus, nutation_terms(:,3), (nutation_terms(:,4) * JCE))) .* cos(tabulated_argument);
...
mean_obliquity = (((((((((p(1)*U + p(2)).*U + p(3)).*U + p(4)).*U + p(5)).*U + p(6)).*U + p(7)).*U + p(8)).*U + p(9)).*U + p(10)).*U + p(11);
...
observer_local_hour = apparent_stime_at_greenwich + bsxfun(@minus, location.longitude, sun_rigth_ascension);
...
idx = (var<min_interval);
var(idx) = var(idx) + max_interval;
