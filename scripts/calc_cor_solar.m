function [su, cor_solar] = calc_cor_solar (data, s, meta, ind)
  ...
  if ~isempty (su)
    ...
    first_solar_data = find (s.solar_users == s_su(1));
...
%}
      ...
      if max (fd) ~= length (fd)
        ...
        for r = 1:size(rr, 2)
          ...
          minus_vamp = s.daily_min(solar_user_range(i), rr(1,r):rr(2,r)) - min(data(su(i),rr(1,r):rr(2,r),[1:8*meta.SamPerDay/48,floor(43*meta.SamPerDay/48):meta.SamPerDay]), [], 3);
          if rr(2,r) - rr(1,r) > 8 * meta.SamPerDay/48
...
%}
    ...
    elseif learn_fail_mode == 2
      ...
      figure(101); plot (1:size(s.daily_min,2), s.daily_min (solar_user_range(i),:))
    ...
    default_val = NaN;    % 0 also makes sense
    for i = find (first_solar_day ~= 1)'
      solar_correction(i,1:first_solar_day(i)-1,:) = default_val;
    end
    for i = find (last_solar_day ~= meta.Days)'
      solar_correction(i,last_solar_day(i)+1:end,:) = default_val;
    end
    for i = find (any (discon, 2))'
      ...
      solar_correction(i, discon(i,:), :) = default_val;
      cvt_cor = data(i, :, :);
      cvt_cor(:, :, s.dark_end:s.dark_start) = cvt_cor(:, :, s.dark_end:s.dark_start) + solar_correction(i,:,:);
...
%     solar_correction = bsxfun (@times, solar_correction, factor);
...
%}
    ...
    tmp(:, :, s.dark_end:s.dark_start) = solar_correction;
    idx = ~isnan (solar_correction(:));
    tmp = max (tmp, 0);
